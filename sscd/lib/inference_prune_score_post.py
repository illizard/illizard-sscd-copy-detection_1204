...
from sscd.models.score_post_eval_model_og_ft import Model
    ...
    def inference(cls, args, dataset, base_name="predictions"):
        if args.features:
            ...
            if args.checkpoint:
                ...
                flops = cls.perform_head_pruning(pl_model.model, args, 64)  # 헤드 프루닝 적용
...
# 이상없음, 값 전부 일일히 확인해봄 
    ...
    def perform_head_pruning(model, args, head_size):
        if args.based == 'score':
            ...
            # flops = perform_head_pruning_and_cal_flops(model, sorted_data_dict, head_size)
