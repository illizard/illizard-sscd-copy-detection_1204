...
class InferenceModel(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx):
        ...
        # https://github.com/PyTorchLightning/pytorch-lightning/issues/11287
        batch = {k: v.cpu() for (k, v) in batch.items()}
        return batch # batch.keys() {'instance_id': tensor([1]), 'split': tensor([0]), 'image_num': tensor([277154]), 'embeddings': tensor([[ 3.3409e-02...956e-02]]), 'attention_maps': [tensor([[[[0.0195, 0...='cuda:1'), tensor([[[[0.0099, 0...='cuda:1'), tensor([[[[0.0229, 0...='cuda:1'), tensor([[[[0.0117, 0...='cuda:1'), tensor([[[[0.0155, 0...='cuda:1'), tensor([[[[0.0059, 0...='cuda:1'), tensor([[[[0.0093, 0...='cuda:1'), tensor([[[[0.0104, 0...='cuda:1'), tensor([[[[0.0042, 0...='cuda:1'), ...]}
...
class Inference:
    ...
    def inference(cls, args, dataset, base_name="predictions"):
        ...
        #             batch_data = {k: v for (k, v) in batch.items() if k in ["image_num", "split", "instance_id"]}
        #             batch_data["embeddings"] = embeddings.cpu()
        #             outputs.append(batch_data)
        #     logger.info("Loading features")
        #     # 출력들을 하나로 합치기
        #     outputs = coalesce_outputs(outputs)
        # else:
        #     raise ValueError("Either --checkpoint or --features is required")

        logger.info("Deduplication") #outputs['attention_maps'].shape = torch.Size([45360, 197, 197])
