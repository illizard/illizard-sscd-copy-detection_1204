...
from sscd.models.score_pre_eval_model_og_ft import Model
...
class Inference:
    ...
    def inference(cls, args, dataset, base_name="predictions"):
        if args.features:
            ...
            if args.checkpoint:
                ...
                # MODI 
                cls.perform_head_pruning(pl_model.model, args.block_idx, args.head_idx, 64)  # 헤드 프루닝 적용
    ...
    @staticmethod
    def perform_head_pruning(model, layer_index, head_index, head_size):
        # qkv 가중치와 바이어스에 접근
        qkv_weight = model.backbone.blocks[layer_index].attn.qkv.weight.data
        qkv_bias = model.backbone.blocks[layer_index].attn.qkv.bias.data
        ...
        model.backbone.blocks[layer_index].attn.qkv.weight.data = torch.cat([q_weight, k_weight, v_weight], 0)
        model.backbone.blocks[layer_index].attn.qkv.bias.data = torch.cat([q_bias, k_bias, v_bias], 0)
