...
class SSCD(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # weight_decay_params = [param for name, param in self.model.named_parameters() if 'qkv.weight' in name and param.shape[0] < 384]
        # no_decay_params = [param for name, param in self.model.named_parameters() if 'qkv.weight' in name and param.shape[0] >= 384]
        # no_decay_params = [param for name, param in self.model.named_parameters() if 'qkv.weight' in name and param.shape[0] < 384]
        # weight_decay_params = [param for name, param in self.model.named_parameters() if 'qkv.weight' in name and param.shape[0] >= 384]
        
        
        # # print(f"no_decay_params is {len(no_decay_params)}")
        # optimizer = LARS([{'params': weight_decay_params, 'weight_decay': self.weight_decay},
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ortho weight_check # modi 231003
        # w_dict = compute_weight_orthogonality(self)

        return self.loss(embeddings, instance_ids)        
        # return self.loss(embeddings, instance_ids, w_dict) # modi 231003
    ...
    def loss(self, embeddings, instance_labels): # modi 231003
    # def loss(self, embeddings, instance_labels, w_dict): # modi 231003
        # print(f"embeddings is {embeddings.shape}")
        # print(f"instance_labels is {instance_labels}")
        # print(self)
        # print()
        ...
        # ## orthogonal loss # modi 231003
        # orthogonal_loss = 0.0
        # lamb = self.lamb
        # for idx_blk in w_dict.values():
        #     orthogonal_loss += idx_blk

        # components["orthogonal"] = lamb*orthogonal_loss
        
        # loss = infonce_loss # OG
        loss = infonce_loss + entropy_loss # OG
        # loss = infonce_loss + entropy_loss + lamb*orthogonal_loss# modi 231003
    ...
    def on_train_end(self):
        ...
        # modi
        path = os.path.join(self.output_path, f"og[model_{self.model_idx}]_{self.backbone}_{self.blk}_{self.orthogonality}_{self.lamb}_{self.token}_{self.desc_size}_ep{self.epochs}_dims_model_torchscript.pt")      
...
def main(args):
	...
	# 		                project='exp_comp')
    wandb_logger = WandbLogger(name=f'og[model_{args.model_idx}]_{args.backbone}_{args.blk}_{args.orthogonality}_{args.lamb}_{args.token}_{args.desc_size}_ep{args.epochs}_{c_date}_{c_time}', 
			                project='exp_test')
